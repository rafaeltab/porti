    ...
    async fn project(&self, event: OrganizationEvent) -> Result<(), Error> {
        match event {
            OrganizationEvent::AddPlatformAccount {
                ...
                self.client.execute("INSERT INTO \"PlatformAccount\" (id, organization_id, name, platform_name) VALUES ($1, $2, $3, $4);", &[&id, &organization_id, &account.name, &account.platform.name]).await
            }
