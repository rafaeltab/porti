    ...
    fn from_json(value: serde_json::Value, event_type: &str) -> EventStoreOrganizationEvent {
        match event_type {
            "Porti.SourceControl/Aggregates/Organization/AddPlatformAccount/1" => {
                let organization_id = &value["organization_id"].as_u64().expect("");
                let account_id = &value["account"]["id"].as_u64().expect("");
                let account_name = &value["account"]["name"].as_str().expect("");
                let platform_name = &value["account"]["platform"]["name"].as_str().expect("");
